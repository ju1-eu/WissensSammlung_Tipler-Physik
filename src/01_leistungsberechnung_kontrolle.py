...
def main():
    ...
    df_ergebnisse = pd.DataFrame({"Variante 0": ergebnis_v0, "Variante 2": ergebnis_v2})

    # Differenzen berechnen
    df_ergebnisse["Differenz"] = (
        df_ergebnisse["Variante 2"] - df_ergebnisse["Variante 0"]
    )
    ...
    df_parameter = pd.DataFrame(parameter_dict, index=["Variante 0", "Variante 2"])

    # Prozentuale Änderungen berechnen
    proz_aenderung = {
        "η_Getriebe": (
            (variante2.eta_getriebe - variante0.eta_getriebe)
            / variante0.eta_getriebe
            * 100
        ),
        "μ": ((variante2.mu - variante0.mu) / variante0.mu * 100),
        "Motorleistung": (
            (ergebnis_v2["P_motor"] - ergebnis_v0["P_motor"])
            / ergebnis_v0["P_motor"]
            * 100
        ),
    }
