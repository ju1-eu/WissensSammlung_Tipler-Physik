...
class FahrwiderstandAnalyse:
    ...
    def linear_interpolation(self, v: np.ndarray) -> np.ndarray:
        ...
        """
        p = self.points
        f_roll = np.where(v <= p.v1, p.f1,
                 np.where(v >= p.v2, p.f2,
                 p.f1 + (p.f2 - p.f1) * (v - p.v1) / (p.v2 - p.v1)))
        return f_roll
    ...
    def update_analysis(self, v_analyse: float):
        ...
        self.ax1.plot(v_full, P_roll_full, '-', color=COLORS['roll'], 
                     label='Rollwiderstand', linewidth=2)
        self.ax1.plot(v_full, P_ges_full, '-', color=COLORS['ges'], 
                     label='Gesamtwiderstand', linewidth=2)
        self.ax1.set_ylim(0, 100)
        self.ax1.set_title('Leistungsbedarf (0-200 km/h)')
        
        # Detailbereich Plot (0-50 km/h)
        self.ax2.plot(v_detail, P_roll_detail, '-', color=COLORS['roll'], 
                     label='Rollwiderstand', linewidth=2)
        self.ax2.plot(v_detail, P_ges_detail, '-', color=COLORS['ges'], 
                     label='Gesamtwiderstand', linewidth=2)
        ...
        if v_analyse <= 200:
            ...
            self.ax1.vlines(v_analyse, P_roll[0], P_ges[0], color=COLORS['diff'],
                          linestyles='--', label=f'Luftwiderstand bei {v_analyse:.0f} km/h')
            
            # Markierung im Detailplot wenn v <= 50
            if v_analyse <= 50:
                self.ax2.vlines(v_analyse, P_roll[0], P_ges[0], color=COLORS['diff'],
                              linestyles='--')
    ...
    def save_analysis(self, output_dir: str = 'output', filename: str = 'fahrwiderstand_analyse'):
        """
